...
**Never hand-write a digest.** `MD5-CAM` is a tamper-detection checksum that CKM/tooling **computes on upload or build** — it is output, not input. A template without a valid digest is still editable and importable (CKM regenerates it); a *wrong* digest is worse than none. Leave the existing value untouched, or omit the block.
...
| `<integrity_checks>` | Per-archetype `MD5-CAM` digest for tamper detection; regenerated by tooling, never authored |
...
`<unitMagnitude>` is repeatable — one per permitted unit — and its bounds may carry `<includesMinimum>` / `<includesMaximum>` booleans for inclusivity. `<includedUnits>` / `<excludedUnits>` list units to permit or forbid.

### countConstraint (DV_COUNT)

Magnitude bounds for a count, using the same bound elements as `unitMagnitude`:
```xml
...
`<includedTypes>` values seen in published CKM OETs: `Text`, `Coded_text`, `Boolean`, `Count`, `Quantity`, `Interval`, `Date_Time`, `Duration`, `Identifier`, `URI`.
