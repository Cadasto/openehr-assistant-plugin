...
### Changed
- **Slash-command surface reduced 20 → 7.** Merged search (`/archetype-search` + `/template-search` → `/ckm-search`), explain/lookup (`/archetype-explain` + `/template-explain` + `/type-spec` + `/rm-structure` + `/adl-idiom` + `/terminology` → `/openehr-explain`), and diff (`/archetype-diff` + `/template-diff` → `/semantic-diff`). Dropped commands that duplicated a skill: `/aql-designer` → `aql-authoring`, `/format-data` → `composition-builder`, `/archetype-review` + `/archetype-rationale` → `archetype-authoring`, `/guide` → `openehr-assistant`, and `/archetype-lint` → the user-invocable `archetype-lint` skill (same name; `/archetype-lint` still resolves to it).
